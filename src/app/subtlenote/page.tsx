...
export default function SubtleNotePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Direct browser submit to Formspree for instant notification & dashboard tracking
      try {
        ...
        console.warn("Formspree direct alert fallback:", fErr);
      }

      // 2. Local API registration
      const res = await fetch("/api/subtlenote-waitlist", {
        method: "POST",
  ...
  const loadAdminData = async () => {
    ...
    try {
      const res = await fetch("/api/subtlenote-waitlist?admin=true");
      const data = await res.json();
      if (data.entries) {
  ...
  return (
    <div className="relative min-h-screen bg-[#09090b] text-[#f4f4f6] antialiased selection:bg-emerald-500/20 selection:text-emerald-300 font-sans">
      ...
      <main className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 pt-12 pb-24">
        ...
        {/* Footer with Assignment Verification Dashboard */}
        ...
        {/* Checkpoint 1 Verification Modal */}
        {showAdminModal && (
