...
const DATA_FILE = path.join(DATA_DIR, "subtlenote-signups.json");
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.error("Formspree forward error:", formspreeErr);
    }
    ...
    if (existing) {
      return NextResponse.json({
        ...
        message: "You're already on the VIP list!",
    ...
    return NextResponse.json({
      ...
      spotsRemaining: Math.max(0, 100 - signups.length),
      totalCount: signups.length,
      message: `You're on the list! We reserved spot #${signups.length} for you.`,
    });
  } catch (error) {
    return NextResponse.json(
